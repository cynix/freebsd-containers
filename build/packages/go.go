...
func (gp *GoProject) BuildPackage(gh *github.Client, version, name string) error {
	pkg, ok := gp.Packages[name]
	if !ok {
		return fmt.Errorf("unknown package: %q", name)
	}

	if err := gp.ApplyPatches(); err != nil {
		return err
	}

	return pkg.Build(name, gp.Arch, gp.Builder == "cgo")
}

func (gp *GoProject) BuildContainer(gh *github.Client, version, name string) error {
	pkg, ok := gp.Packages[name]
	if !ok {
		return fmt.Errorf("unknown package: %q", name)
	}
...
func (gp *GoPackage) Build(name string, arch []string, cgo bool) error {
	gr := goReleaser{
		Version:     2,
		ProjectName: name,
		Dist:        "../dist",
	...
	core.Group("Generating .goreleaser.yaml", func() {
